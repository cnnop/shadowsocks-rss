...
| verify_simple  |  97%  |     96%   |    Yes   |    No    |        No         |            1          |          0          |
| verify_deflate |  50%  |  97%~110% |    Yes   |    No    |        No         |            6          |          0          |
| verify_sha1    |  90%  |   94%/99% |    Yes   |    Yes   |        No         |            0          |          0          |
| auth_simple    |  97%  |     95%   |    Yes   |    No    |        Yes        |            1          |          0          |
| auth_sha1      |  98%  |     90%   |    Yes   |    Yes   |        Yes        |            4          |          0          |
| auth_sha1_v2   |  97%  |     80%   |    Yes   |    Yes   |        Yes        |           10          |          0          |
...
-  auth\_sha1\_v2的bandwidth在浏览普通网页时较低（为了较强的长度混淆，但单个数据包尺寸会保持在1440以内，所以其实对网速影响较小），而看视频或下载时比auth_sha1要高，可达98%，所以不用担心下载时的速度
...
-  如果QoS在你的地区明显，混淆建议在`http_simple`、`http_post`与`tls1.2_ticket_auth`中选择
-  如果你不在乎QoS，但担心你的个人vps能不能持久使用，那么混淆选择`plain`或`tls1.2_ticket_auth`，协议选择`auth_sha1_v2`
